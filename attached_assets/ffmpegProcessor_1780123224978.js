...
// Audio duration cache — avoids creating a new Audio element for every video
...
export const initializeFFmpeg = async () => {
  ...
  initPromise = (async () => {
    try {
      ...
      if (!isLoaded) {
        // Try loading FFmpeg securely from unpkg to prevent Web Worker silent failures
        try {
          DEBUG && console.log('Trying FFmpeg load from unpkg CDN via toBlobURL...');
          const baseURL = 'https://unpkg.com/@ffmpeg/core@0.12.10/dist/esm';
          await ffmpeg.load({
            coreURL: await toBlobURL(`${baseURL}/ffmpeg-core.js`, 'text/javascript'),
            wasmURL: await toBlobURL(`${baseURL}/ffmpeg-core.wasm`, 'application/wasm'),
          });
          DEBUG && console.log('FFmpeg loaded successfully from unpkg CDN');
        } catch (cdnError) {
          console.error('External CDN loading failed:', cdnError);
          // Fallback to default load if CDN fails
          try {
        ...
        // Log whether true multi-threading (SharedArrayBuffer) is available
        console.log('[FFmpeg Optimizer] Loaded with threads:', typeof SharedArrayBuffer !== 'undefined');
  ...
  try {
    ...
    progressHandler = ({ progress, time }) => {
      ...
      if (onProgress && progressCallbackActive && (now - lastProgressTime > 100) && !hasCompleted) {
        ...
        // Fallback: calculate progress from time if progress is undefined, NaN, or 0
        if ((!actualProgress || isNaN(actualProgress) || actualProgress === 0) && time !== undefined && audioDuration > 0) {
          const timeInSeconds = time / 1000000; // time is in microseconds
          actualProgress = timeInSeconds / audioDuration;
        }

        const normalizedProgress = Math.min(Math.max((actualProgress || 0) * 100, 0), 100);
    ...
    // Ensure audioDuration is a valid number to prevent FFmpeg from hanging on NaN/Infinity
    if (!audioDuration || isNaN(audioDuration) || !isFinite(audioDuration) || audioDuration <= 0) {
      console.warn('Invalid audio duration detected, defaulting to 180 seconds');
      audioDuration = 180;
    }
    ...
    // 4K uses single-threaded mode: multi-thread frame buffers at 3840×2160 exhaust
    // the browser WASM memory ceiling with the single-threaded @ffmpeg/core build.
    ...
    ffmpegArgs.push(
      ...
      '-g', '60', '-keyint_min', '60', // Keyframe every 60 frames (12s at 5fps) — reduces overhead for static content
      '-c:a', 'aac',
      ...
      '-threads', threadCount,       // 1 thread for 4K (memory safety), 4 for lower resolutions
      '-shortest',
    ...
    try {
      ...
      if (isWarningError) {
        ...
        // For 4K failures, force-reset the WASM instance so the next video starts clean.
        // A crashed 4K encode leaves the WASM worker in an undefined state; without this
        // the next exec() call on the same instance also fails.
        if (is4K) {
          console.warn('[4K] Exec failed — forcing FFmpeg reinit to prevent state pollution');
          isLoaded = false;
          ffmpeg = null;
          initPromise = null;
        }
...
export const processVideoWithFFmpegInstance = async (
  ...
  try {
    ...
    progressHandler = ({ progress, time }) => {
      ...
      let actualProgress = progress;
      if ((!actualProgress || isNaN(actualProgress) || actualProgress === 0) && time !== undefined && typeof audioDuration !== 'undefined' && audioDuration > 0) {
        actualProgress = (time / 1000000) / audioDuration;
      }
      const pct = Math.min(Math.max((actualProgress || 0) * 100, 0), 99);
      onProgress(pct);
    };
    ...
    const threadCount4k = (quality === '4k') ? '1' : '4';
    ffmpegArgs.push(
      '-vf', videoFilter,
      ...
      '-threads', threadCount4k, '-shortest', '-t', audioDuration.toString(), '-y', outputFileName
    );
...
export const getAudioDuration = (audioFile) => {
  const cacheKey = `${audioFile.name}_${audioFile.size}_${audioFile.lastModified}`;
  if (audioDurationCache.has(cacheKey)) {
    return Promise.resolve(audioDurationCache.get(cacheKey));
  }
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      console.warn('getAudioDuration timeout, using default 180s');
    ...
    audio.onloadedmetadata = () => {
      ...
      audioDurationCache.set(cacheKey, audio.duration);
      if (objectUrl) URL.revokeObjectURL(objectUrl);
    ...
    audio.onerror = (error) => {
      ...
      if (objectUrl) URL.revokeObjectURL(objectUrl);
      resolve(180); // Resolve to default instead of rejecting to allow video generation to proceed
    };
