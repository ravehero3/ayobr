...
export const useFFmpeg = () => {
    ...
    try {
      ...
      if (maxConcurrent === 1) {
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
          ...
          if (batchIndex < batches.length - 1 && !useAppStore.getState().isCancelling) await new Promise(r => setTimeout(r, 50));
        }
      ...
      const { resetGenerationState, setIsGenerating: setStoreIsGenerating } = useAppStore.getState();
  ...
  }, [setIsGenerating, addGeneratedVideo, clearGeneratedVideos, setVideoGenerationState, resetCancellation]);
  ...
  const processPairAsync = async (pair) => {
    try {
      ...
      if (useAppStore.getState().isCancelling || (error && error.message === 'Generation cancelled by user')) {
      ...
      if (isRestartableError && !useAppStore.getState().isCancelling) {
