...
function App({ onBeforeGenerate }) {
  ...
  const handleGenerateVideos = async () => {
    ...
    // Before generating, check credits/permissions via callback
    if (onBeforeGenerate) {
      const allowed = await onBeforeGenerate(completePairs.length);
      if (!allowed) return;
    }
          ...
          >
            <div className="text-center">
              <motion.div
                ...
                animate={{ scale: 1 }}
                exit={{ scale: 0.8 }}
                className="inline-flex items-center justify-center w-32 h-32 mb-6 rounded-full bg-white/5 border-4 border-dashed border-white/20"
              >
