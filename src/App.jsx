  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 flex flex-col bg-overlay" style={{ zIndex: 2 }}>
        ...
        {/* Always visible drop zone - centered when no files, compact at top when files exist */}
        <motion.div
        ...
        >
          <div className={`inline-flex items-center justify-center rounded-full bg-blue-400/10 border-2 border-dashed border-blue-400/30 mb-4 ${
            pairs.every(pair => !pair.audio && !pair.image) 
              ? 'w-24 h-24 mb-6' 
              : 'w-16 h-16'
          }`}>
            <svg className={`text-blue-400/50 fill-none stroke-currentColor ${
              pairs.every(pair => !pair.audio && !pair.image) 
                ? 'w-12 h-12' 
                : 'w-8 h-8'
            }`} viewBox="0 0 24 24">
