  ...
  return (
    <>
      ...
      <button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        className={className} style={{ ...style, position: 'relative', overflow: 'visible', fontFamily: NM }}>
        {particles.map(p => (
          <span key={p.id} style={{
...
/* Stat with "up to" label above in script font */
function Stat({ prefix, val, label }) {
  return (
    <div className="text-center">
      {prefix && (
        <div style={{ fontFamily: SCRIPT, fontSize: '0.85rem', color: 'rgba(255,255,255,0.38)', lineHeight: 1, marginBottom: 2 }}>
          {prefix}
        </div>
      )}
      <div style={{ fontFamily: NM, fontWeight: 900, fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', lineHeight: 1, letterSpacing: '-0.04em', color: '#fff' }}>
        {val}
      </div>
      <div style={{ fontFamily: NM, fontSize: '0.8rem', color: 'rgba(255,255,255,0.38)', marginTop: 5, letterSpacing: '0.02em' }}>
        {label}
      </div>
    </div>
  );
...
const features = [
  ...
  { icon: '🖼️', title: 'Custom video backgrounds', desc: 'PRO users can upload any photo as a custom background. Free plan includes clean black & white backgrounds.' },
  ...
  { icon: '🖥️', title: 'Runs entirely in your browser', desc: 'No upload limits, no waiting on servers. All video processing happens locally on your machine.' },
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050a13] text-white overflow-x-hidden" style={{ fontFamily: NM }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4"
        ...
        <div className="flex items-center gap-4">
          <a href="#pricing" style={{ fontFamily: NM }} className="text-sm text-gray-400 hover:text-white transition-colors">Pricing</a>
          {user ? (
            <button onClick={() => navigate('/app')} style={{ fontFamily: NM }}
              className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-sm font-medium transition-colors">
              Open App
            </button>
          ) : (
            <button onClick={login} style={{ fontFamily: NM }}
              className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-sm font-medium transition-colors">
              Sign In
            </button>
          )}
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 text-center pt-20" style={{ overflow: 'hidden' }}>
        ...
        <div className="absolute inset-0 pointer-events-none" style={{
          zIndex: 1,
          background: 'radial-gradient(ellipse 80% 80% at 50% 50%, transparent 40%, rgba(5,10,19,0.55) 65%, rgba(5,10,19,0.92) 85%, #050a13 100%)'
        }} />

        {/* Edge vignette — left & right */}
        <div className="absolute inset-0 pointer-events-none" style={{
          zIndex: 1,
          background: 'linear-gradient(to right, #050a13 0%, transparent 12%, transparent 88%, #050a13 100%)'
        }} />

        {/* Top & bottom edges */}
        <div className="absolute inset-0 pointer-events-none" style={{
          zIndex: 1,
          background: 'linear-gradient(to bottom, #050a13 0%, transparent 10%, transparent 80%, #050a13 100%)'
        }} />
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-300 text-sm mb-8"
            style={{ fontFamily: NM }}>
            <span>🎬</span>
            <span style={{ fontFamily: SCRIPT, fontSize: '1rem', letterSpacing: '0.01em' }}>The fastest way to fill your YouTube channel.</span>
          </div>
          ...
          <p style={{ fontFamily: NM, fontSize: 'clamp(1rem, 2vw, 1.2rem)', color: 'rgba(255,255,255,0.5)', maxWidth: '38rem', margin: '0 auto 2.5rem', lineHeight: 1.65 }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <ParticleButton onClick={handleCTA}
              ...
              style={{ background: 'linear-gradient(135deg, #3b82f6, #0ea5e9)', boxShadow: '0 0 40px rgba(59,130,246,0.3)', fontFamily: NM, fontWeight: 700 }}>
              {user ? 'Open the App' : 'Get Started Free'}
            </ParticleButton>
            <a href="#how-it-works"
              className="px-8 py-4 rounded-xl text-lg border border-white/10 hover:border-white/30 transition-colors"
              style={{ fontFamily: NM, fontWeight: 600 }}>
              See how it works
            </a>
          </div>

          <p style={{ fontFamily: NM, marginTop: '1rem', fontSize: '0.8rem', color: 'rgba(255,255,255,0.28)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
          className="relative flex flex-wrap justify-center gap-12 mt-20" style={{ zIndex: 2 }}>
          <Stat prefix="up to" val="100" label="videos per one batch" />
          <Stat prefix="auto" val="50" label="pairs matched" />
          <Stat prefix="up to" val="4K" label="video quality" />
          <Stat val="320kbps" label="audio quality" />
        </motion.div>
      </section>
      ...
      <section className="py-24 px-6 max-w-6xl mx-auto">
        ...
        <p style={{ fontFamily: NM, color: 'rgba(255,255,255,0.45)', textAlign: 'center', marginBottom: '4rem', maxWidth: '36rem', margin: '0 auto 4rem' }}>
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: '#050a13', minHeight: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div className="text-center p-12">
              ...
              <div className="flex items-center justify-center gap-3 mt-6">
                <span className="px-3 py-1 rounded-full border border-white/10 text-xs text-gray-400" style={{ fontFamily: NM }}>🎵 MP3, WAV</span>
                <span className="px-3 py-1 rounded-full border border-white/10 text-xs text-gray-400" style={{ fontFamily: NM }}>🖼️ PNG, JPG</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="pricing" className="py-24 px-6 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          ...
          <div className="rounded-2xl p-8 border border-white/10" style={{ background: 'rgba(255,255,255,0.03)' }}>
            ...
            <div style={{ fontFamily: NM, fontWeight: 900, fontSize: '2.6rem', letterSpacing: '-0.05em', marginBottom: '1.5rem' }}>
              $0<span style={{ fontSize: '1rem', fontWeight: 400, color: 'rgba(255,255,255,0.4)' }}>/mo</span>
            </div>
            <ul className="space-y-3 text-sm text-gray-300 mb-8">
              {[
                ...
                <li key={item} className="flex items-center gap-2" style={{ fontFamily: NM }}>
          ...
          <div className="rounded-2xl p-8 border border-blue-500/40 relative overflow-hidden"
            ...
            <div className="absolute top-4 right-4 px-2 py-1 rounded-full bg-blue-500 text-xs font-bold" style={{ fontFamily: NM }}>POPULAR</div>
            <h3 style={{ fontFamily: NM, fontWeight: 800, fontSize: '1.2rem', letterSpacing: '-0.02em', marginBottom: '0.5rem' }}>PRO</h3>
            <div style={{ fontFamily: NM, fontWeight: 900, fontSize: '2.6rem', letterSpacing: '-0.05em', marginBottom: '1.5rem' }}>
              $9.99<span style={{ fontSize: '1rem', fontWeight: 400, color: 'rgba(255,255,255,0.4)' }}>/mo</span>
            </div>
            <ul className="space-y-3 text-sm text-gray-300 mb-8">
              {[
            ...
            <button onClick={handleUpgradeCTA}
              ...
              style={{ background: 'linear-gradient(135deg, #3b82f6, #0ea5e9)', fontFamily: NM, fontWeight: 700 }}>
      ...
      <section className="py-24 px-6">
        <div className="max-w-3xl mx-auto text-center rounded-2xl p-12 border border-white/10"
          ...
          <h2 style={{ fontFamily: NM, fontWeight: 900, marginBottom: '1rem', fontSize: 'clamp(1.6rem, 4vw, 2.5rem)', lineHeight: 0.95, letterSpacing: '-0.03em' }}>
            Ready to make type beats<br />in batches and save hours<br />
            <span style={{ fontFamily: SCRIPT, fontSize: '0.7em', fontWeight: 400, color: 'rgba(255,255,255,0.6)', WebkitTextFillColor: 'rgba(255,255,255,0.6)' }}>
              of your life?
            </span>
          </h2>
          <p style={{ fontFamily: NM, color: 'rgba(255,255,255,0.45)', marginBottom: '2rem' }}>
            Join producers who are uploading more beats while doing less work.
          </p>
          <ParticleButton onClick={handleCTA}
            className="px-10 py-4 rounded-xl text-lg transition-all hover:scale-105"
            style={{ background: 'linear-gradient(135deg, #3b82f6, #0ea5e9)', boxShadow: '0 0 40px rgba(59,130,246,0.3)', fontFamily: NM, fontWeight: 700 }}>
      ...
      <footer className="py-8 px-6 border-t border-white/[0.06] text-center text-gray-500 text-sm">
        <div className="flex flex-col md:flex-row items-center justify-between max-w-5xl mx-auto gap-4" style={{ fontFamily: NM }}>
