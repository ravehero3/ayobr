  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[10000] flex items-center justify-between px-6 h-14"
      ...
      <div className="flex items-center gap-3">
          ...
          <>
            ...
            {onUpgradePro && (
              <button onClick={onUpgradePro} disabled={checkoutLoading}
                ...
                {checkoutLoading ? (
                  <><span className="inline-block w-3 h-3 border border-white/40 border-t-white rounded-full animate-spin" />Opening...</>
                ) : 'Go PRO'}
              </button>
            )}
          </>
        ...
        {/* UNLIMITED badge */}
        {user && isUnlimited && (
          <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border border-yellow-500/40 text-yellow-300"
                ...
                <div className="absolute right-0 top-10 z-50 w-52 rounded-xl border border-white/10 py-1 shadow-2xl"
                  ...
                  {!isPaidPlan && onUpgradeUnlimited && (
                    <button onClick={() => { onUpgradeUnlimited(); setMenuOpen(false); }}
                      className="w-full text-left px-4 py-2 text-sm text-yellow-400 hover:text-yellow-300 hover:bg-white/5 transition-colors">
                      🌟 Go Unlimited
                    </button>
                  )}
