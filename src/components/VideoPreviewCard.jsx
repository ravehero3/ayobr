...
const VideoPreviewCard = ({ video }) => {
  ...
  const handleVideoError = () => {
    setError(true);
    console.error('Video playback error for:', video.filename);
  };
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 shadow-lg border border-gray-700">
      {error ? (
        <div className="w-full h-48 bg-gray-900 rounded mb-3 flex items-center justify-center">
      ...
      <div className="text-white mb-3">
        <h3 className="font-semibold truncate text-sm" title={video.filename}>
          {video.filename}
        </h3>
        <div className="text-xs text-gray-400 space-y-1">
          <p>Size: {formatFileSize(video.size || video.blob?.size || 0)}</p>
          <p>Created: {new Date(video.createdAt).toLocaleString()}</p>
          {isPlaying && <p className="text-green-400">▶ Playing</p>}
        </div>
      </div>
...
The code has been modified by replacing the original `VideoPreviewCard` component with a new one that includes improved error handling, video controls, and styling. Additionally, the missing `useState` import has been added to the `React` import statement.
```replit_final_file
import React, { useState, useRef, useEffect } from 'react';
import { motion } from 'framer-motion';
import { useVideoPlayback } from '../hooks/useVideoPlayback';
