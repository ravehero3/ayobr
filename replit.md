...
- **Modular Pair Containers**: Individual containers for audio and images, allowing drag-and-drop swapping only between same-type containers (audio with audio, image with image). Containers are 384px x 192px with a consistent glassmorphism design. Subtle hover effects scale containers 2% larger for visual feedback. When dragging, containers of the same type shimmer and levitate to indicate valid drop targets without intrusive stroke effects.
