...
### UI/UX Decisions
The application features a futuristic dark theme with deep navy and matte black backgrounds, accented by neon blue glow effects and subtle highlights. The application now uses a sophisticated 4-page system with distinct background transitions: Page 1 (Upload) uses a dark gradient background, Page 2 (File Management) displays the dynamic blue flame GIF (typebeatznew_1754459272105.gif) with diagonal movement animation, Page 3 (Generation) shows the same GIF but with blur effects during processing, and Page 4 (Download) features a special dark blue gradient for video preview and download. Background preloading ensures smooth transitions between all states. Containers are designed with rounded corners (24-32px) and a glassmorphism effect, including semi-transparent dark backgrounds, 16px backdrop blur, and a noise texture overlay for a modern aesthetic. UI elements like the play button are circular, and audio waveform visualizations are clean and resemble the GNOME Decibels app interface. Text colors are light gray for readability. Animations utilize Framer Motion for smooth transitions, including merge animations where containers move to center during video generation, ambient border pulses, flare flickers, and enhanced drag-and-drop visual feedback (e.g., green borders, scaling effects, and 10-degree tilt for image containers). Button styling includes spotlight effects with color variants: blue for generation, red for stopping, and green for downloading.
